...
class control extends pluginbase {
	...
	function onindex() {
		...
		foreach($showlist as $file => $md5) {
			...
			if(@array_key_exists($file, $modifylist)) {
				...
				$filemtime = @UC::file()->filemtime($file);
			...
			if(UC::file()->file_exists($file)) {
				$filemtime = @UC::file()->filemtime($file);
				$fileststus && $dirlist[$fileststus][$dir][basename($file)] = array(number_format(UC::file()->filesize($file)).' Bytes', $this->date($filemtime));
			} else {
	...
	function checkfiles($currentdir, $ext = '', $sub = 1, $skip = '') {
		$dir = @UC::file()->opendir(UC_ROOT.$currentdir);
		$exts = '/('.$ext.')$/i';
		$skips = explode(',', $skip);

		while($entry = @UC::file()->readdir($dir)) {
			$file = $currentdir.$entry;
			if($entry != '.' && $entry != '..' && (preg_match($exts, $entry) || $sub && UC::file()->is_dir($file)) && !in_array($entry, $skips)) {
				if($sub && UC::file()->is_dir($file)) {
