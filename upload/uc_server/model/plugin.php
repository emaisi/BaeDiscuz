...
class pluginmodel {
	...
	function get_plugins() {
		...
		$arr = array();
		$dir = UC_ROOT.'./plugin';
		$d = UC::file()->opendir($dir);
		while($f = UC::file()->readdir($d)) {
			if($f != '.' && $f != '..' && $f != '.svn' && UC::file()->is_dir($dir.'/'.$f)) {
				$s = UC::file()->file_get_contents($dir.'/'.$f.'/plugin.xml');
	...
	function get_plugin($pluginname) {
		$f = UC::file()->file_get_contents(UC_ROOT."./plugin/$pluginname/plugin.xml");
		include_once UC_ROOT.'./lib/xml.class.php';
	...
	function get_plugin_by_name($pluginname) {
		$dir = UC_ROOT.'./plugin';
		$s = UC::file()->file_get_contents($dir.'/'.$pluginname.'/plugin.xml');
	...
	function cert_dump_decode($certfile) {
		$s = @UC::file()->file_get_contents($certfile);
		if(empty($s)) {
			return array();
		}
		preg_match("/# Expires: (.*?)\n/", $s, $m);
		if(empty($m[1]) || $m[1] < $this->time) {
			UC::file()->unlink($certfile);
			return array();
		}
		$s = preg_replace("/(#.*\s+)*/", '', $s);
