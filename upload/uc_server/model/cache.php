	...
	function updatedata($cachefile = '') {
		if($cachefile) {
			foreach((array)$this->map[$cachefile] as $modules) {
				...
				$s .= "\r\n?>";
				@UC::file()->file_put_contents(UC_DATADIR."./cache/$cachefile.php", $s);
			}
		} else {
			foreach((array)$this->map as $file => $modules) {
				...
				$s .= "\r\n?>";
				@UC::file()->file_put_contents(UC_DATADIR."./cache/$file.php", $s);
			}
		}
	}

	function updatetpl() {
		$tpl = UC::file()->opendir(UC_DATADIR.'view');
		while($entry = UC::file()->readdir($tpl)) {
			if(preg_match("/\.php$/", $entry)) {
				@UC::file()->unlink(UC_DATADIR.'view/'.$entry);
			}
		}
		$tpl->close();
	}
