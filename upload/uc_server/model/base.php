...
class base {
	...
	function load($model, $base = NULL, $release = '') {
		...
		if(empty($_ENV[$model])) {
			...
			if(UC::file()->file_exists(UC_ROOT.$release."model/$model.php")) {
	...
	function &cache($cachefile) {
		...
		if(!isset($_CACHE[$cachefile])) {
			...
			if(!UC::file()->file_exists($cachepath)) {
				$this->load('cache');
				$_ENV['cache']->updatedata($cachefile);
			} else {
				include_once UC::file()->save_file($cachepath);
			}
		}
	...
	function serialize($s, $htmlon = 0) {
		if(UC::file()->file_exists(UC_ROOT.RELEASE_ROOT.'./lib/xml.class.php')) {
