...
class adminbase extends base {
	...
	function writelog($action, $extra = '') {
		...
		if(@UC::file()->filesize($logfile) > 2048000) {
		...
		if($fp = @UC::file()->fopen($logfile, 'a')) {
			@UC::file()->flock($fp, 2);
			@UC::file()->fwrite($fp, "<?PHP exit;?>\t".str_replace(array('<?', '?>', '<?php'), '', $log)."\n");
			@UC::file()->fclose($fp);
		}
	}
	...
	function fetch_plugins() {
		...
		$d = UC::file()->opendir($plugindir);
		while($f = UC::file()->readdir($d)) {
			if($f != '.' && $f != '..' && UC::file()->is_dir($plugindir.'/'.$f)) {
