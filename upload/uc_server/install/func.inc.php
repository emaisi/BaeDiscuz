	...
	foreach($dirfile_items as $key => $item) {
		...
		if($item['type'] == 'dir') {
			if(!dir_writeable(ROOT_PATH.$item_path)) {
				if(UC::file()->is_dir(ROOT_PATH.$item_path)) {
			...
			if(UC::file()->file_exists(ROOT_PATH.$item_path)) {
				if(UC::file()->is_writable(ROOT_PATH.$item_path)) {
					$dirfile_items[$key]['status'] = 1;
					$dirfile_items[$key]['current'] = '+r+w';
				} else {
					$dirfile_items[$key]['status'] = 0;
...
function dir_writeable($dir) {
	$writeable = 0;
	if(!UC::file()->is_dir($dir)) {
		@mkdir($dir, 0777);
	}
	if(UC::file()->is_dir($dir)) {
		if($fp = @UC::file()->fopen("$dir/test.txt", 'w')) {
			@UC::file()->fclose($fp);
			@UC::file()->unlink("$dir/test.txt");
			$writeable = 1;
		} else {
			$writeable = 0;
		}
...
function dir_clear($dir) {
	...
	while($entry = $directory->read()) {
		...
		if(UC::file()->is_file($filename)) {
			@UC::file()->unlink($filename);
		}
	}
	$directory->close();
	@UC::file()->touch($dir.'/index.htm');
}
...
function loginit($logfile) {
	...
	if($fp = @UC::file()->fopen('./forumdata/logs/'.$logfile.'.php', 'w')) {
		UC::file()->fwrite($fp, '<'.'?PHP exit(); ?'.">\n");
		UC::file()->fclose($fp);
	}
}
...
function config_edit() {
	...
	$fp = UC::file()->fopen(CONFIG, 'w');
	UC::file()->fwrite($fp, $config);
	UC::file()->fclose($fp);
}
...
function dopen($url, $limit = 0, $post = '', $cookie = '', $bysocket = FALSE, $ip = '', $timeout = 15, $block = TRUE) {
	$return = '';
	...
	if(!$fp) {
		...
		if(!$status['timed_out']) {
			...
			$stop = false;
			while(!feof($fp) && !$stop) {
				$data = UC::file()->fread($fp, ($limit == 0 || $limit > 8192 ? 8192 : $limit));
				$return .= $data;
				if($limit) {
					$limit -= strlen($data);
					$stop = $limit <= 0;
...
function check_env() {
	...
	if(!UC::file()->file_exists(DISCUZ_ROOT.'./config.inc.php')) {
...
function save_uc_config($config, $file) {
	...
	if($content = UC::file()->file_get_contents($file)) {
		...
		if(@UC::file()->file_put_contents($file, $content)) {
			$success = true;
		}
	}
