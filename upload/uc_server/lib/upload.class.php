...
Class upload{
	...
	function mkdir_by_date($date, $dir = '.') {
		...
		!UC::file()->is_dir("$dir/$y") && mkdir("$dir/$y", 0777);
		!UC::file()->is_dir("$dir/$y/$m$d") && mkdir("$dir/$y/$m$d", 0777);
		return "$y/$m$d";
	}

	function mkdir_by_hash($s, $dir = '.') {
		$s = md5($s);
		!UC::file()->is_dir($dir.'/'.$s[0]) && mkdir($dir.'/'.$s[0], 0777);
		!UC::file()->is_dir($dir.'/'.$s[0].'/'.$s[1]) && mkdir($dir.'/'.$s[0].'/'.$s[1], 0777);
		!UC::file()->is_dir($dir.'/'.$s[0].'/'.$s[1].'/'.$s[2]) && mkdir($dir.'/'.$s[0].'/'.$s[1].'/'.$s[2], 0777);
		return $s[0].'/'.$s[1].'/'.$s[2];
	}
	...
	function mkdir_by_uid($uid, $dir = '.') {
		...
		$dir2 = substr($uid, 3, 2);
		$dir3 = substr($uid, 5, 2);
		!UC::file()->is_dir($dir.'/'.$dir1) && mkdir($dir.'/'.$dir1, 0777);
		!UC::file()->is_dir($dir.'/'.$dir1.'/'.$dir2) && mkdir($dir.'/'.$dir1.'/'.$dir2, 0777);
		!UC::file()->is_dir($dir.'/'.$dir1.'/'.$dir2.'/'.$dir3) && mkdir($dir.'/'.$dir1.'/'.$dir2.'/'.$dir3, 0777);
		return $dir1.'/'.$dir2.'/'.$dir3;
	}

	function copy($sourcefile, $destfile) {
		move_uploaded_file($sourcefile, $destfile);
		@UC::file()->unlink($sourcefile);
	}
	...
	function watermark($target, $watermark_file, $ext, $watermarkstatus=9, $watermarktrans=50) {
		...
		if($gdsurporttype && in_array($ext, $gdsurporttype) ) {
			$attachinfo	= getimagesize(C::file()->save_file($target));
			...
			if($attachinfo['mime'] == 'image/gif') {
				$fp = UC::file()->fopen($target, 'rb');
				$targetcontent = UC::file()->fread($fp, 9999999);
				UC::file()->fclose($fp);
	...
	function thumb($forcedwidth, $forcedheight, $sourcefile, $destfile, $destext, $imgcomp = 0) {
		...
		if(UC::file()->file_exists($g_srcfile)) {
			$g_is = getimagesize($g_srcfile);
			if($g_is[0] < $forcedwidth && $g_is[1] < $forcedheight) {
				copy($sourcefile, $destfile);
				return UC::file()->filesize($destfile);;
			}
