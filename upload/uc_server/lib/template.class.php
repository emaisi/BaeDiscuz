...
class template {
	...
	function gettpl($file) {
		...
		$tplfilemtime = @UC::file()->filemtime($this->tplfile);
		if($tplfilemtime === FALSE) {
			$this->tplfile = $this->defaulttpldir.'/'.$file.'.htm';
		}
		if($this->force || !UC::file()->file_exists($this->objfile) || @UC::file()->filemtime($this->objfile) < UC::file()->filemtime($this->tplfile)) {
		...
		return UC::file()->save_file($this->objfile);
	}

	function complie() {
		$template = UC::file()->file_get_contents($this->tplfile);
		...
		$fp = UC::file()->fopen($this->objfile, 'w');
		UC::file()->fwrite($fp, $template);
		UC::file()->fclose($fp);
	}
