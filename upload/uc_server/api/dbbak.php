...
if($get['method'] == 'export') {
	...
	} elseif(!UC::file()->is_dir(BACKUP_DIR.'./'.$get['sqlpath'])) {
		if(!mkdir(BACKUP_DIR.'./'.$get['sqlpath'], 0777)) {
			api_msg('mkdir_error', 'make dir error:'.BACKUP_DIR.'./'.$get['sqlpath']);
		}		
	}
	...
	if(trim($sqldump)) {
		$sqldump = "$idstring".
			...
			"# --------------------------------------------------------\n\n\n".
			$sqldump;
		@$fp = UC::file()->fopen($dumpfile, 'wb');
		@UC::file()->flock($fp, 2);
		if(@!UC::file()->fwrite($fp, $sqldump)) {
			@UC::file()->fclose($fp);
		...
		@UC::file()->touch(ROOT_PATH.$get['sqlpath'].'/index.htm');
	...
	if(!UC::file()->is_file(BACKUP_DIR.$get['sqlpath'].'/'.$get['dumpfile'])) {
		if(UC::file()->is_file(BACKUP_DIR.$get['sqlpath'].'/'.$next_dumpfile)) {
	...
	$sqldump = UC::file()->file_get_contents(BACKUP_DIR.$get['sqlpath'].'/'.$get['dumpfile']);
	...
	if($get['dir'] && UC::file()->is_dir(BACKUP_DIR.$get['dir'])) {
		echo "1";exit;
	} else {
		echo "-1";exit;
	}
	...
	while($entry = $directory->read()) {
		...
		if(UC::file()->is_dir($filename) && preg_match('/backup_(\d+)_\w+$/', $filename, $match)) {
			$str .= "\t<dir>\n";
	...
	if(empty($sqlpath) || !UC::file()->is_dir(BACKUP_DIR.$sqlpath)) {
	...
	while($entry = $directory->read()) {
		...
		if(UC::file()->is_file($filename) && preg_match('/\d+_\w+\-(\d+).sql$/', $filename, $match)) {
			$str .= "\t<file>\n";
			$str .= "\t\t<file_name>$match[0]</file_name>\n";
			$str .= "\t\t<file_size>".UC::file()->filesize($filename)."</file_size>\n";
			$str .= "\t\t<file_num>$match[1]</file_num>\n";
			$str .= "\t\t<file_url>".str_replace(ROOT_PATH, 'http://'.$_SERVER['HTTP_HOST'].'/', $filename)."</file_url>\n";
			$str .= "\t\t<last_modify>".UC::file()->filemtime($filename)."</last_modify>\n";
			$str .= "\t</file>\n";
		}
	}
	$directory->close();
	...
	while($entry = $directory->read()) {
		$filename = BACKUP_DIR.$sqlpath.'/'.$entry;
		if(UC::file()->is_file($filename) && preg_match('/\d+_\w+\-(\d+).sql$/', $filename) && !@UC::file()->unlink($filename)) {
