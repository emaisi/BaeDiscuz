...
class control extends adminbase {
	...
	function onls() {
		...
		if($operate == 'list') {
			if($delete = $_POST['delete']) {
				if(is_array($delete)) {
					foreach($delete AS $filename) {
						@UC::file()->unlink('./data/backup/'.str_replace(array('/', '\\'), '', $filename));
					}
				}
				$status = 2;
			...
			$baklist = array();
			if(UC::file()->is_dir(UC_ROOT.'./data/backup/')) {
				$dir = dir(UC_ROOT.'./data/backup/');
				while($entry = $dir->read()) {
					$file = './data/backup/'.$entry;
					if(UC::file()->is_dir($file) && preg_match("/backup_(\d+)_\w+/i", $file, $match)) {
	...
	function onoperate() {
		...
		if($nexturl) {
			...
			if($appid) {
				...
				$url = 'http://'.$_SERVER['HTTP_HOST'].str_replace('admin.php', 'api/dbbak.php', $_SERVER['REQUEST_URI']).'?apptype=UCENTER';
