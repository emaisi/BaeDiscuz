...
class control extends adminbase {
	...
	function onsync() {
		...
		if(!$step && is_array($this->cache['apps'])) {
			...
			if(!$stepapp) {
				...
				include UC::file()->save_file(UC_DATADIR.'cache/credits.php');
			}
			if($app = $this->cache['apps'][$appid]) {
				...
				if($data) {
					...
					$s .= "\r\n?>";
					$fp = @UC::file()->fopen(UC_DATADIR.'cache/credits.php', 'w');
					@UC::file()->fwrite($fp, $s);
					@UC::file()->fclose($fp);
				}
		...
		include_once UC::file()->save_file(UC_DATADIR.'cache/credits.php');
	...
	function sync_unserialize($s, $release_root) {
		if(!function_exists('xml_unserialize')) {
			if($release_root && UC::file()->file_exists(UC_ROOT.$release_root.'./lib/xml.class.php')) {
