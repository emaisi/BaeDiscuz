...
class usercontrol extends base {
	...
	function onuploadavatar() {
		...
		UC::file()->file_exists($tmpavatar) && @UC::file()->unlink($tmpavatar);
		$tmp = sys_get_temp_dir().'/upload'.$uid.$filetype;
		if(@copy($_FILES['Filedata']['tmp_name'], $tmp) || @move_uploaded_file($_FILES['Filedata']['tmp_name'], $tmp)) {
			@UC::file()->unlink($_FILES['Filedata']['tmp_name']);
		...
		UC::file()->upload($tmp, $tmpavatar);
		$avatarurl = UC::file()->url(UC_DATADIR.'/tmp/upload'.$uid.$filetype);
		return $avatarurl;
	}
	...
	function onrectavatar() {
		...
		if(!UC::file()->is_dir(UC_DATADIR.'./avatar/'.$home)) {
		...
		$success = 1;
		$fp = @UC::file()->fopen($bigavatarfile, 'wb');
		@UC::file()->fwrite($fp, $bigavatar);
		@UC::file()->fclose($fp);

		$fp = @UC::file()->fopen($middleavatarfile, 'wb');
		@UC::file()->fwrite($fp, $middleavatar);
		@UC::file()->fclose($fp);

		$fp = @UC::file()->fopen($smallavatarfile, 'wb');
		@UC::file()->fwrite($fp, $smallavatar);
		@UC::file()->fclose($fp);

		$biginfo = @getimagesize(UC::file()->save_file($bigavatarfile));
		$middleinfo = @getimagesize(UC::file()->save_file($middleavatarfile));
		$smallinfo = @getimagesize(UC::file()->save_file($smallavatarfile));
		if(!$biginfo || !$middleinfo || !$smallinfo || $biginfo[2] == 4 || $middleinfo[2] == 4 || $smallinfo[2] == 4) {
			UC::file()->file_exists($bigavatarfile) && UC::file()->unlink($bigavatarfile);
			UC::file()->file_exists($middleavatarfile) && UC::file()->unlink($middleavatarfile);
			UC::file()->file_exists($smallavatarfile) && UC::file()->unlink($smallavatarfile);
			$success = 0;
		}

		$filetype = '.jpg';
		@UC::file()->unlink(UC_DATADIR.'./tmp/upload'.$uid.$filetype);
