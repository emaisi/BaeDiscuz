...
if($apptype == 'discuz') {
	...
	require C::file()->save_file(ROOT_PATH.'./config/config_global.php');
	require C::file()->save_file(ROOT_PATH.'./config/config_ucenter.php');
} else {
...
if($get['method'] == 'export') {
	...
	} elseif(!C::file()->is_dir(BACKUP_DIR.'./'.$get['sqlpath'])) {
		if(!mkdir(BACKUP_DIR.'./'.$get['sqlpath'], 0777)) {
			api_msg('mkdir_error', 'make dir error:'.BACKUP_DIR.'./'.$get['sqlpath']);
		}
	}
	...
	if(trim($sqldump)) {
		...
		@C::file()->touch(ROOT_PATH.$get['sqlpath'].'/index.htm');
	...
	if(!C::file()->is_file(BACKUP_DIR.$get['sqlpath'].'/'.$get['dumpfile'])) {
		if(C::file()->is_file(BACKUP_DIR.$get['sqlpath'].'/'.$next_dumpfile)) {
	...
	$sqldump = C::file()->file_get_contents(BACKUP_DIR.$get['sqlpath'].'/'.$get['dumpfile']);
	...
	if($get['dir'] && C::file()->is_dir(BACKUP_DIR.$get['dir'])) {
		echo "1";exit;
	} else {
		echo "-1";exit;
	}
	...
	while($entry = $directory->read()) {
		...
		if(C::file()->is_dir($filename) && preg_match('/backup_(\d+)_\w+$/', $filename, $match)) {
			$str .= "\t<dir>\n";
	...
	if(empty($sqlpath) || !C::file()->is_dir(BACKUP_DIR.$sqlpath)) {
	...
	while($entry = $directory->read()) {
		...
		if(C::file()->is_file($filename) && preg_match('/\d+_\w+\-(\d+).sql$/', $filename, $match)) {
			$str .= "\t<file>\n";
			$str .= "\t\t<file_name>$match[0]</file_name>\n";
			$str .= "\t\t<file_size>".C::file()->filesize($filename)."</file_size>\n";
			$str .= "\t\t<file_num>$match[1]</file_num>\n";
			$str .= "\t\t<file_url>".str_replace(ROOT_PATH, 'http://'.$_SERVER['HTTP_HOST'].'/', $filename)."</file_url>\n";
			$str .= "\t\t<last_modify>".C::file()->filemtime($filename)."</last_modify>\n";
			$str .= "\t</file>\n";
		}
	}
	$directory->close();
	...
	while($entry = $directory->read()) {
		$filename = BACKUP_DIR.$sqlpath.'/'.$entry;
		if(C::file()->is_file($filename) && preg_match('/\d+_\w+\-(\d+).sql$/', $filename) && !@C::file()->unlink($filename)) {
...
function auto_next($get, $sqlfile) {
	$next_url = 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'].'?apptype='.$GLOBALS['apptype'].'&code='.urlencode(encode_arr($get));
	$out = "<root>\n";
	$out .= "\t<error errorCode=\"0\" errorMessage=\"ok\" />\n";
	$out .= "\t<fileinfo>\n";
	$out .= "\t\t<file_num>$get[volume]</file_num>\n";
	$out .= "\t\t<file_size>".C::file()->filesize($sqlfile)."</file_size>\n";
	$out .= "\t\t<file_name>".basename($sqlfile)."</file_name>\n";
	$out .= "\t\t<file_url>".str_replace(ROOT_PATH, 'http://'.$_SERVER['HTTP_HOST'].'/', $sqlfile)."</file_url>\n";
	$out .= "\t\t<last_modify>".C::file()->filemtime($sqlfile)."</last_modify>\n";
	$out .= "\t</fileinfo>\n";
