...
if(!defined('IN_UC')) {
	...
	require C::file()->save_file(DISCUZ_ROOT.'./config/config_ucenter.php');
	...
	$post = xml_unserialize(C::file()->file_get_contents('php://input'));
	...
	function updateapps($get, $post) {
		...
		if($UC_API /*&& is_writeable(DISCUZ_ROOT.'./config/config_ucenter.php')*/) {
			if(preg_match('/^https?:\/\//is', $UC_API)) {
				$configfile = trim(C::file()->file_get_contents(DISCUZ_ROOT.'./config/config_ucenter.php'));
				$configfile = substr($configfile, -2) == '?>' ? substr($configfile, 0, -2) : $configfile;
				$configfile = preg_replace("/define\('UC_API',\s*'.*?'\);/i", "define('UC_API', '".addslashes($UC_API)."');", $configfile);
				if($fp = @C::file()->fopen(DISCUZ_ROOT.'./config/config_ucenter.php', 'w')) {
