	...
	foreach($dirfile_items as $key => $item) {
		...
		if($item['type'] == 'dir') {
			...
			if(file_exists(ROOT_PATH.$item_path)) {
				if(C::file()->is_writable(ROOT_PATH.$item_path)) {
					$dirfile_items[$key]['status'] = 1;
					$dirfile_items[$key]['current'] = '+r+w';
				} else {
					$dirfile_items[$key]['status'] = 0;
...
EOT;
	...
	$bcsfile->file_put_contents($filename, $content);
}
...
define('UC_DBTABLEPRE', '$uctablepre');
...
?>
EOT;
	if($fp = $bcsfile->fopen($file, 'w')) {
		$bcsfile->fwrite($fp, $config);
		$bcsfile->fclose($fp);
		$success = true;
	}
	return $success;
...
function install_uc_server() {
	...
	$appurl = 'http://'.preg_replace("/\:\d+/", '', $_SERVER['HTTP_HOST']).$pathinfo['dirname'];
...
function save_diy_data($primaltplname, $targettplname, $data, $database = false) {
	...
	$r = $bcsfile->file_put_contents($tplfile, $content);
