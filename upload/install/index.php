...
require ROOT_PATH.'./bcs/class_bcsfile.php';
...
$bcsfile = new bcsfile();
//设置根目录
$bcsfile->set_root_path(ROOT_PATH);
//设置需要写到云存储的目录
$bcsfile->add_remote_dir(ROOT_PATH.'./data/');
$bcsfile->add_remote_dir(ROOT_PATH.'./config/config_global.php');
$bcsfile->add_remote_dir(ROOT_PATH.'./config/config_ucenter.php');
$bcsfile->add_remote_dir(ROOT_PATH.'./uc_client/data/');
$bcsfile->add_remote_dir(ROOT_PATH.'./uc_server/data/');
//设置可写的临时目录
$bcsfile->set_temp_dir(sys_get_temp_dir());
/*
...
if(in_array($method, array('app_reg', 'ext_info'))) {
	$REQUEST_URI = $_SERVER['REQUEST_URI'] ? $_SERVER['REQUEST_URI'] : $_SERVER['SCRIPT_NAME'];
	...
	$default_appurl = $bbserver.substr($REQUEST_URI, 0, strrpos($REQUEST_URI, '/') - 8);
}
...
if($method == 'show_license') {
	...
	if($submit) {
		...
		$appurl = strtolower(($_SERVER['HTTPS'] == 'on' ? 'https' : 'http').'://'.$_SERVER['HTTP_HOST'].substr($_SERVER['REQUEST_URI'], 0, strrpos($_SERVER['REQUEST_URI'], '/install')));
			
		$_config['output']['staticurl'] = $appurl.'/static/';	// 站点静态文件路径，“/”结尾
