...
class helper_log {
	...
	public static function runlog($file, $message, $halt=0) {
		...
		$nowurl = $_SERVER['REQUEST_URI']?$_SERVER['REQUEST_URI']:($_SERVER['REQUEST_URI']?$_SERVER['REQUEST_URI']:$_SERVER['SCRIPT_NAME']);
	...
	public static function writelog($file, $log) {
		...
		if(@C::file()->filesize($logfile) > 2048000) {
			$dir = C::file()->opendir($logdir);
			$length = strlen($file);
			$maxid = $id = 0;
		...
		if($fp = @C::file()->fopen($logfile, 'a')) {
			@C::file()->flock($fp, 2);
			if(!is_array($log)) {
				$log = array($log);
			}
			foreach($log as $tmp) {
				C::file()->fwrite($fp, "<?PHP exit;?>\t".str_replace(array('<?', '?>'), '', $tmp)."\n");
			}
			C::file()->fclose($fp);
		}
	}
