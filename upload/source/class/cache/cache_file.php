...
class ultrax_cache {
	...
	function set_cache($key, $value, $life) {
		...
		if($fp = @C::file()->fopen($cache_file, 'wb')) {
			C::file()->fwrite($fp, "<?php\n//Discuz! cache file, DO NOT modify me!".
				"\n//Created: ".date("M j, Y, G:i").
				"\n//Identify: ".md5($cache_file.$cachedata.$_G['config']['security']['authkey'])."\n\nif(!defined('IN_DISCUZ')) {\n\texit('Access Denied');\n}\n\n$cachedata?>");
			C::file()->fclose($fp);
		} else {
			exit('set_cache()..Can not write to cache files, please check file:'.$cache_file);
		}
		return true;
	}
	...
	function del_cache($key) {
		$cache_file = $this->get_cache_file_path($key);
		if(C::file()->file_exists($cache_file)) {
			return @C::file()->unlink($cache_file);
		}
		return true;
	}
	...
	function cache_exists($key) {
		...
		$data = $this->_get_cache($key);
		if($data['life'] && (C::file()->filemtime($cache_file) < time() - $data['life'])) {
			return false;
		}
		return true;
	}
