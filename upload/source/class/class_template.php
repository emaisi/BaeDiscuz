...
class template {
	...
	function parse_template($tplfile, $templateid, $tpldir, $file, $cachefile) {
		...
		if($fp = @C::file()->fopen(DISCUZ_ROOT.$tplfile, 'r')) {
			$template = @C::file()->fread($fp, C::file()->filesize(DISCUZ_ROOT.$tplfile));
			C::file()->fclose($fp);
		} elseif($fp = @C::file()->fopen($filename = substr(DISCUZ_ROOT.$tplfile, 0, -4).'.php', 'r')) {
			$template = $this->getphptemplate(@C::file()->fread($fp, C::file()->filesize($filename)));
			C::file()->fclose($fp);
		} else {
			$tpl = $tpldir.'/'.$file.'.htm';
		...
		if(!@$fp = C::file()->fopen(DISCUZ_ROOT.$cachefile, 'w')) {
	...
	function loadsubtemplate($file) {
		...
		if(($content = @implode('', C::file()->file($filename))) || ($content = $this->getphptemplate(@implode('', C::file()->file(substr($filename, 0, -4).'.php'))))) {
	...
	function loadcsstemplate() {
		...
		$scriptcss = '<link rel="stylesheet" type="text/css" href="'.C::file()->url().'/data/cache/style_{STYLEID}_common.css?{VERHASH}" />';
		$content = $this->csscurmodules = '';
		$content = @implode('', C::file()->file('/data/cache/style_'.STYLEID.'_module.css'));
		...
		if($this->csscurmodules) {
			...
			if(@$fp = C::file()->fopen(DISCUZ_ROOT.'./data/cache/style_'.STYLEID.'_'.$_G['basescript'].'_'.CURMODULE.'.css', 'w')) {
				C::file()->fwrite($fp, $this->csscurmodules);
				C::file()->fclose($fp);
			} else {
				exit('loadcsstemplate()...Can not write to cache files, please check file:'.DISCUZ_ROOT.'./data/cache/style_'.STYLEID.'_'.$_G['basescript'].'_'.CURMODULE.'.css');
			}
			$scriptcss .= '<link rel="stylesheet" type="text/css" href="'.C::file()->url().'/data/cache/style_{STYLEID}_'.$_G['basescript'].'_'.CURMODULE.'.css?{VERHASH}" />';
		}
		$scriptcss .= '{if $_G[uid] && isset($_G[cookie][extstyle]) && strpos($_G[cookie][extstyle], TPLDIR) !== false}<link rel="stylesheet" id="css_extstyle" type="text/css" href="'.C::file()->url().'$_G[cookie][extstyle]/style.css" />{elseif $_G[style][defaultextstyle]}<link rel="stylesheet" id="css_extstyle" type="text/css" href="$_G[style][defaultextstyle]/style.css" />{/if}';
		return $scriptcss;
	}
