...
class table_common_syscache extends discuz_table
{
	...
	public function fetch_all($cachenames) {
		...
		if($this->_isfilecache) {
			...
			foreach($cachenames as $cachename) {
				if(!@include_once C::file()->save_file(DISCUZ_ROOT.'./data/cache/cache_'.$cachename.'.php')) {
		...
		while($syscache = DB::fetch($query)) {
			...
			if($this->_isfilecache) {
				...
				if(($fp = @C::file()->fopen(DISCUZ_ROOT.'./data/cache/cache_'.$syscache['cname'].'.php', 'wb'))) {
	...
	public function insert($cachename, $data) {
		...
		$this->_isfilecache && @C::file()->unlink(DISCUZ_ROOT.'./data/cache/cache_'.$cachename.'.php');
	}
