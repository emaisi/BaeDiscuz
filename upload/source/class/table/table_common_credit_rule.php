...
class table_common_credit_rule extends discuz_table
{
	...
	public function fetch_all_by_rid($rid = 0) {
		$parameter = array(DB::table($this->_table));
		$wherearr = array();
		if($rid) {
	...
	public function fetch_all_rule() {
		return DB::fetch_all('SELECT * FROM '.DB::table($this->_table).' ORDER BY rid DESC');
	}

	public function fetch_all_by_action($action) {
		if(!empty($action)) {
			return DB::fetch_all('SELECT * FROM '.DB::table($this->_table). ' WHERE action IN(%n)', array($action), $this->_pk);
		}
		return array();
	}
