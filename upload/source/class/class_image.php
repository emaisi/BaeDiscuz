...
class image {
	...
	function Watermark($source, $target = '', $type = 'forum') {
		...
		if(!C::file()->file_exists($this->param['watermarkfile'][$type]) || ($this->param['watermarktype'][$type] == 'text' && (!C::file()->file_exists($this->param['watermarktext']['fontpath'][$type]) || !C::file()->is_file($this->param['watermarktext']['fontpath'][$type])))) {
	...
	function init($method, $source, $target, $nosuffix = 0) {
		...
		if(isset($parse['host'])) {
			...
			C::file()->file_put_contents($source, $data);
		...
		clearstatcache();
		if(!C::file()->file_exists($source) || !C::file()->is_writable($targetpath)) {
			return -2;
		}

		$imginfo = @getimagesize(C::file()->save_file($source));
		if($imginfo === FALSE) {
			return -1;
		}
		...
		$this->imginfo['size'] = @C::file()->filesize($source);
		...
		if(!$this->libmethod && $this->imginfo['mime'] == 'image/gif') {
			...
			if(!($fp = @C::file()->fopen($source, 'rb'))) {
				return -2;
			}
			$content = C::file()->fread($fp, $this->imginfo['size']);
			C::file()->fclose($fp);
	...
	function sleep($return) {
		if($this->tmpfile) {
			@C::file()->unlink($this->tmpfile);
		}
		$this->imginfo['size'] = @C::file()->filesize($this->target);
		return $this->returncode($return);
	}

	function returncode($return) {
		if($return > 0 && C::file()->file_exists($this->target)) {
			return true;
		} else {
	...
	function loadsource() {
		...
		if(!$im) {
			...
			$fp = @C::file()->fopen($this->source, 'rb');
			$contents = @C::file()->fread($fp, C::file()->filesize($this->source));
			C::file()->fclose($fp);
		...
		switch($this->param['thumbtype']) {
			...
			case 1:
				if($this->imginfo['width'] > $this->param['thumbwidth'] || $this->imginfo['height'] > $this->param['thumbheight']) {
					...
					if(!C::file()->file_exists($this->target)) {
						return -3;
					}
				}
				break;
	...
	function Watermark_GD($type = 'forum') {
		...
		if($this->param['watermarktype'][$type] != 'text') {
			...
			$watermarkinfo = @getimagesize(C::file()->save_file($this->param['watermarkfile'][$type]));
