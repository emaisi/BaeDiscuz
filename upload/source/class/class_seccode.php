...
class seccode {
	...
	function background() {
		...
		if($this->background && function_exists('imagecreatefromjpeg') && function_exists('imagecolorat') && function_exists('imagecopymerge') &&
			...
			if($handle = @C::file()->opendir($this->datapath.'background/')) {
				while($bgfile = @C::file()->readdir($handle)) {
					if(preg_match('/\.jpg$/i', $bgfile)) {
						$backgrounds[] = $this->datapath.'background/'.$bgfile;
					}
				}
				@C::file()->closedir($handle);
			}
			if($backgrounds) {
	...
	function giffont() {
		...
		if(function_exists('imagecreatefromgif')) {
			...
			$dirs = C::file()->opendir($seccoderoot);
			while($dir = C::file()->readdir($dirs)) {
				if($dir != '.' && $dir != '..' && C::file()->file_exists($seccoderoot.$dir.'/9.gif')) {
					$seccodedir[] = $dir;
				}
			}
		}
		...
		for($i = 0; $i <= 3; $i++) {
			...
			if(!empty($this->imcodefile) && C::file()->file_exists($this->imcodefile)) {
				$font[$i]['file'] = $this->imcodefile;
				$font[$i]['data'] = getimagesize(C::file()->save_file($this->imcodefile));
	...
	function audio() {
		...
		for($i = 0;$i <= 3; $i++) {
			C::file()->readfile($this->datapath.'sound/'.strtolower($this->code[$i]).'.mp3');
		}
	}
