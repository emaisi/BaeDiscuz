...
class discuz_application extends discuz_base{
	...
	private function _init_env() {
		...
		$_G['REQUEST_URI'] = dhtmlspecialchars($this->_get_script_url());
		$_G['basescript'] = CURSCRIPT;
		$_G['basefilename'] = basename($_G['REQUEST_URI']);
		$sitepath = substr($_G['REQUEST_URI'], 0, strrpos($_G['REQUEST_URI'], '/'));
		if(defined('IN_API')) {
	...
	private function _get_script_url() {
		if(!isset($this->var['REQUEST_URI'])){
			$scriptName = basename($_SERVER['SCRIPT_FILENAME']);
			if(basename($_SERVER['SCRIPT_NAME']) === $scriptName) {
				$this->var['REQUEST_URI'] = $_SERVER['SCRIPT_NAME'];
			} else if(basename($_SERVER['REQUEST_URI']) === $scriptName) {
				$this->var['REQUEST_URI'] = $_SERVER['REQUEST_URI'];
			} else if(isset($_SERVER['ORIG_SCRIPT_NAME']) && basename($_SERVER['ORIG_SCRIPT_NAME']) === $scriptName) {
				$this->var['REQUEST_URI'] = $_SERVER['ORIG_SCRIPT_NAME'];
			} else if(($pos = strpos($_SERVER['REQUEST_URI'],'/'.$scriptName)) !== false) {
				$this->var['REQUEST_URI'] = substr($_SERVER['SCRIPT_NAME'],0,$pos).'/'.$scriptName;
			} else if(isset($_SERVER['DOCUMENT_ROOT']) && strpos($_SERVER['SCRIPT_FILENAME'],$_SERVER['DOCUMENT_ROOT']) === 0) {
				$this->var['REQUEST_URI'] = str_replace('\\','/',str_replace($_SERVER['DOCUMENT_ROOT'],'',$_SERVER['SCRIPT_FILENAME']));
				$this->var['REQUEST_URI'][0] != '/' && $this->var['REQUEST_URI'] = '/'.$this->var['REQUEST_URI'];
			} else {
				system_error('request_tainting');
			}
		}
		return $this->var['REQUEST_URI'];
	}
	...
	private function _init_config() {
		...
		$_config = array();
		@include C::file()->save_file(DISCUZ_ROOT.'./config/config_global.php');
		$this->var['fileurl'] = C::file()->url();

		if(empty($_config['security']['authkey'])) {
			$_config['security']['authkey'] = md5($_config['cookie']['cookiepre'].$_config['db'][1]['dbname']);
		}

		if(empty($_config['debug']) || !C::file()->file_exists(libfile('function/debug'))) {
	...
	private function _init_misc() {
		...
		$lastact = TIMESTAMP."\t".dhtmlspecialchars(basename($this->var['REQUEST_URI']))."\t".dhtmlspecialchars($this->var['mod']);
