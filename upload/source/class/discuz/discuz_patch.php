...
class discuz_patch {
	...
	public function fix_patch($patch, $type = 'file') {
		...
		foreach($rules as $rule) {
			...
			if(!$siteftp && !C::file()->is_writable($filename)) {
				$returnflag = -2;
				break;
			}

			$str = C::file()->file_get_contents($filename);
			...
			if(!C::file()->file_put_contents($tmpfile, $replacestr)) {
				$returnflag = -3;
				break;
			}

			if($siteftp) {
				if(!C::file()->file_exists(DISCUZ_ROOT.$bakfile) && !$this->copy_file($filename, $bakfile, 'ftp')) {
					$returnflag = -4;
					break;
				}
				$i = 0;
				...
				if(!C::file()->file_exists($bakfile) && !$this->copy_file($filename, $bakfile, 'file')) {
					$returnflag = -5;
					break;
				}
				$i = 0;
	...
	public function test_writable($sdir) {
		...
		$dir = C::file()->opendir($sdir);
		while($entry = C::file()->readdir($dir)) {
			$file = $sdir.$entry;
			if($entry != '.' && $entry != '..') {
				if(C::file()->is_dir($file) && !strrpos($file.'/', '.svn')) {
		...
		if($fp = @C::file()->fopen("$sdir/test.txt", 'w')) {
			@C::file()->fclose($fp);
			@C::file()->unlink("$sdir/test.txt");
			$writeable = true;
		} else {
			$writeable = false;
		}
	...
	public function test_patch_writable($patch) {
		...
		if($rules) {
			foreach($rules as $rule) {
				if(!C::file()->is_writable(DISCUZ_ROOT.$rule['filename'])) {
					return false;
				}
			}
			return true;
