...
class discuz_upgrade {
	...
	public function fetch_updatefile_list($upgradeinfo) {
		...
		$upgradedata = @C::file()->file_get_contents($file);
		if(!$upgradedata) {
		...
		if(!$upgradedataflag) {
			...
			$fp = C::file()->fopen($file, 'w');
			if(!$fp) {
				return array();
			}
			C::file()->fwrite($fp, $upgradedata);
		}
	...
	public function compare_file_content($file, $remotefile) {
		...
		$content = preg_replace('/\s/', '', C::file()->file_get_contents($file));
		$ctx = stream_context_create(array('http' => array('timeout' => 60)));
		$remotecontent = preg_replace('/\s/', '', C::file()->file_get_contents($remotefile, false, $ctx));
	...
	public function check_folder_perm($updatefilelist) {
		foreach($updatefilelist as $file) {
			if(!C::file()->file_exists(DISCUZ_ROOT.$file)) {
				if(!$this->test_writable(dirname(DISCUZ_ROOT.$file))) {
					return false;
				}
			} else {
				if(!C::file()->is_writable(DISCUZ_ROOT.$file)) {
					return false;
				}
			}
		}
	...
	public function test_writable($dir) {
		...
		if(C::file()->is_dir($dir)) {
			if($fp = @C::file()->fopen("$dir/test.txt", 'w')) {
				@C::file()->fclose($fp);
				@C::file()->unlink("$dir/test.txt");
				$writeable = 1;
			} else {
				$writeable = 0;
			}
	...
	public function mkdirs($dir) {
		if(!C::file()->is_dir($dir)) {
			...
			@C::file()->touch($dir.'/index.htm'); @chmod($dir.'/index.htm', 0777);
		}
		return true;
	}
