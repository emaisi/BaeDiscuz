...
Class discuz_upload{
	...
	function save($ignore = 0) {
		...
		if(empty($this->attach) || empty($this->attach['tmp_name']) || empty($this->attach['target'])) {
			...
			$this->errorcode = -104;
			@C::file()->unlink($this->attach['target']);
		} else {
			$this->errorcode = 0;
			return true;
		}
	...
	function get_image_info($target, $allowswf = false) {
		...
		if(!$isimage && ($ext != 'swf' || !$allowswf)) {
			return false;
		} elseif(!C::file()->file_exists($target)) {
			return false;
		} elseif($imageinfo = @getimagesize(C::file()->save_file($target))) {
	...
	function check_dir_exists($type = '', $sub1 = '', $sub2 = '') {
		...
		$res = $subdir2 ? C::file()->is_dir($subdir2) : ($subdir1 ? C::file()->is_dir($subdir1) : C::file()->is_dir($typedir));
		if(!$res) {
	...
	function save_to_local($source, $target) {
		if(!discuz_upload::is_upload_file($source)) {
			...
			$succeed = true;
		}elseif (@C::file()->file_exists($source) && (@$fp_s = C::file()->fopen($source, 'rb')) && (@$fp_t = C::file()->fopen($target, 'wb'))) {
			while (!feof($fp_s)) {
				$s = @C::file()->fread($fp_s, 1024 * 512);
				@C::file()->fwrite($fp_t, $s);
			}
			C::file()->fclose($fp_s); C::file()->fclose($fp_t);
			$succeed = true;
		}
		if($succeed)  {
			$this->errorcode = 0;
			@chmod($target, 0644); @C::file()->unlink($source);
		} else {
			$this->errorcode = 0;
		}
		return $succeed;
	...
	function make_dir($dir, $index = true) {
		...
		if(!C::file()->is_dir($dir)) {
			$res = @mkdir($dir, 0777);
			$index && @C::file()->touch($dir.'/index.html');
		}
		return $res;
	}
}
