...
if(!$attach['remote'] && !C::file()->file_exists($filename)) {
...
if(!$requestmode && !$range && empty($_GET['noupdate'])) {
	if($_G['setting']['delayviewcount']) {
		...
		if(@$fp = C::file()->fopen(DISCUZ_ROOT.$_G['forum_logfile'], 'a')) {
			C::file()->fwrite($fp, "$aid\n");
			C::file()->fclose($fp);
		} elseif($_G['adminid'] == 1) {
...
$filesize = !$attach['remote'] ? C::file()->filesize($filename) : $attach['filesize'];
...
function getremotefile($file) {
	global $_G;
	@set_time_limit(0);
	if(!@C::file()->readfile($_G['setting']['ftp']['attachurl'].'forum/'.$file)) {
		$ftp = ftpcmd('object');
		$tmpfile = @tempnam($_G['setting']['attachdir'], '');
		if($ftp->ftp_get($tmpfile, 'forum/'.$file, FTP_BINARY)) {
			@C::file()->readfile($tmpfile);
...
function getlocalfile($filename, $readmod = 2, $range = 0) {
	if($readmod == 1 || $readmod == 3 || $readmod == 4) {
		if($fp = @C::file()->fopen($filename, 'rb')) {
			@fseek($fp, $range);
			if(function_exists('fpassthru') && ($readmod == 3 || $readmod == 4)) {
				@fpassthru($fp);
			} else {
				echo @C::file()->fread($fp, C::file()->filesize($filename));
			}
		}
		@C::file()->fclose($fp);
	} else {
		@C::file()->readfile($filename);
	}
	@flush(); @ob_flush();
}
