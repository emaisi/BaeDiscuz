...
function save_diy_data($tpldirectory, $primaltplname, $targettplname, $data, $database = false, $optype = '') {
	...
	if(!C::file()->file_exists($file)) return false;
	$content = C::file()->file_get_contents(DISCUZ_ROOT.$file);
	if($isextphp) {
	...
	if($flag) {
		...
		@C::file()->unlink('./data/diy/'.$tpldirectory.'/'.$targettplname.'_diy_preview.htm');
	}

	$tplfile =DISCUZ_ROOT.'./data/diy/'.$tpldirectory.'/'.$targettplname.'.htm';
	$tplpath = dirname($tplfile);
	if (!C::file()->is_dir($tplpath)) {
		dmkdir($tplpath);
	} else {
		if (C::file()->file_exists($tplfile) && !$flag) copy($tplfile, $tplfile.'.bak');
	}
	$r = C::file()->file_put_contents($tplfile, $content);
	if ($r && $database && !$flag) {
...
function gettheme($type) {
	...
	foreach ($themedirs as $key => $dirname) {
		...
		if(C::file()->file_exists($now_dir.'/style.css') && C::file()->file_exists($now_dir.'/preview.jpg')) {
			$themes[] = array(
				'dir' => $type.'/'.$dirname,
...
function getcssname($dirname) {
	$css = @C::file()->file_get_contents(DISCUZ_ROOT.'./static/'.$dirname.'/style.css');
	if($css) {
...
function import_diy($file) {
	...
	$content = C::file()->file_get_contents($file);
...
function checkprimaltpl($template) {
	...
	if (!C::file()->file_exists($primaltplname)) {
	...
	if (!C::file()->is_file($primaltplname)) {
...
function updatetopic($topic = ''){
	...
	if($primaltplname && empty($topic['primaltplname'])) {
		...
		$content = C::file()->file_get_contents(DISCUZ_ROOT.$tpldirectory.'/'.$primaltplname.'.htm');
		$tplfile = DISCUZ_ROOT.'./data/diy/'.$tpldirectory.'/portal/portal_topic_content_'.$topicid.'.htm';
		$tplpath = dirname($tplfile);
		if (!C::file()->is_dir($tplpath)) {
			dmkdir($tplpath);
		}
		C::file()->file_put_contents($tplfile, $content);
	}
...
function getprimaltplname($filename) {
	...
	$content = @C::file()->file_get_contents(DISCUZ_ROOT.$tpldirectory.'/'.$filename);
