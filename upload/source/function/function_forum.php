...
function dunlink($attach) {
	...
	if($remote) {
		...
		@C::file()->unlink($_G['setting']['attachdir'].'/forum/'.$filename);
		$havethumb && @C::file()->unlink($_G['setting']['attachdir'].'/forum/'.getimgthumbname($filename));
	}
	if($attach['aid']) {
		@C::file()->unlink($_G['setting']['attachdir'].'image/'.$attach['aid'].'_100_100.jpg');
	}
}
