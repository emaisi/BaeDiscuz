...
function checktplrefresh($maintpl, $subtpl, $timecompare, $templateid, $cachefile, $tpldir, $file) {
	...
	if(empty($timecompare) || $tplrefresh == 1 || ($tplrefresh > 1 && !($timestamp % $tplrefresh))) {
		if(empty($timecompare) || @C::file()->filemtime(DISCUZ_ROOT.$subtpl) > $timecompare) {
...
function template($file, $templateid = 0, $tpldir = '', $gettplfile = 0, $primaltpl='') {
	...
	if(strpos($file, ':') !== false) {
		...
		if($templateid == 'diy') {
			...
			$tplsavemod = 0;
			if(isset($diytemplatename[$file]) && C::file()->file_exists($diypath.$file.'.htm') && ($tplsavemod = 1) || empty($_G['forum']['styleid']) && ($file = $primaltpl ? $primaltpl : $oldfile) && isset($diytemplatename[$file]) && C::file()->file_exists($diypath.$file.'.htm')) {
				...
				if(isset($_GET['diy']) && $_GET['diy'] == 'yes' || isset($_GET['diy']) && $_GET['preview'] == 'yes') { //DIY模式或预览模式下做以下判断
					$flag = C::file()->file_exists($diypath.$file.$preend.'.htm');
					if($_GET['preview'] == 'yes') {
						$file .= $flag ? $preend : '';
					} else {
			...
			if($indiy && ($tplrefresh ==1 || ($tplrefresh > 1 && !($_G['timestamp'] % $tplrefresh))) && C::file()->filemtime($diypath.$file.'.htm') < C::file()->filemtime(DISCUZ_ROOT.$_G['style']['tpldirectory'].'/'.($primaltpl ? $primaltpl : $oldfile).'.htm')) {
				if (!updatediytemplate($file, $_G['style']['tpldirectory'])) {
					C::file()->unlink($diypath.$file.'.htm');
					$tpldir = '';
				}
			}
	...
	if(defined('IN_MOBILE') && !defined('TPL_DEFAULT')) {
		if(strpos($tpldir, 'plugin')) {
			if(!C::file()->file_exists(DISCUZ_ROOT.$tpldir.'/'.$file.'.htm') && !C::file()->file_exists(DISCUZ_ROOT.$tpldir.'/'.$file.'.php')) {
		...
		if(strpos($tpldir, 'plugin') && (C::file()->file_exists(DISCUZ_ROOT.$mobiletplfile) || C::file()->file_exists(substr(DISCUZ_ROOT.$mobiletplfile, 0, -4).'.php'))) {
			$tplfile = $mobiletplfile;
		} elseif(!C::file()->file_exists(DISCUZ_ROOT.TPLDIR.'/'.$mobiletplfile) && !C::file()->file_exists(substr(DISCUZ_ROOT.TPLDIR.'/'.$mobiletplfile, 0, -4).'.php')) {
			$mobiletplfile = './template/default/'.$mobiletplfile;
			if(!C::file()->file_exists(DISCUZ_ROOT.$mobiletplfile) && !$_G['forcemobilemessage']) {
	...
	if($templateid != 1 && !C::file()->file_exists(DISCUZ_ROOT.$tplfile) && !C::file()->file_exists(substr(DISCUZ_ROOT.$tplfile, 0, -4).'.php')
			&& !C::file()->file_exists(DISCUZ_ROOT.($tplfile = $tpldir.$filebak.'.htm'))) {
	...
	checktplrefresh($tplfile, $tplfile, @C::file()->filemtime(DISCUZ_ROOT.$cachefile), $templateid, $cachefile, $tpldir, $file);
	
	return C::file()->save_file(DISCUZ_ROOT.$cachefile);
}
...
function loaducenter() {
	require_once C::file()->save_file(DISCUZ_ROOT.'./config/config_ucenter.php');
...
function output() {
	...
	if(defined('CACHE_FILE') && CACHE_FILE && !defined('CACHE_FORBIDDEN') && !defined('IN_MOBILE') && !checkmobile()) {
		if(diskfreespace(DISCUZ_ROOT.'./'.$_G['setting']['cachethreaddir']) > 1000000) {
			if($fp = @C::file()->fopen(CACHE_FILE, 'w')) {
				C::file()->flock($fp, LOCK_EX);
				C::file()->fwrite($fp, empty($content) ? ob_get_contents() : $content);
			}
			@C::file()->fclose($fp);
...
function pluginmodule($pluginid, $type) {
	...
	if(@!C::file()->file_exists(DISCUZ_ROOT.($modfile = './source/plugin/'.$directory.$module.'.inc.php'))) {
...
function dmkdir($dir, $mode = 0777, $makeindex = TRUE){
	if(!C::file()->is_dir($dir)) {
		...
		@mkdir($dir, $mode);
		if(!empty($makeindex)) {
			@C::file()->touch($dir.'/index.html'); @chmod($dir.'/index.html', 0777);
		}
	}
	return true;
}
