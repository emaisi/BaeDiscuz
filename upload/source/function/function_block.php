						...
						if($blockitem['makethumb'] == 1) {
							...
							if($_G['setting']['ftp']['on']) {
								...
								if($ftp->connectid && $ftp->ftp_size($thumbpath) > 0 || ($return = $image->Thumb($replacevalue, $thumbpath, $block['picwidth'], $block['picheight'], 2) && $ftp->upload($_G['setting']['attachurl'].'/'.$thumbpath, $thumbpath))) {
									...
									@C::file()->unlink($_G['setting']['attachdir'].'./'.$thumbpath);
								}
							} elseif(C::file()->file_exists($_G['setting']['attachdir'].$thumbpath) || ($return = $image->Thumb($replacevalue, $thumbpath, $block['picwidth'], $block['picheight'], 2))) {
	...
	for($i = 1; $i <= $block['shownum']; $i++) {
		...
		if(!$jump) {
			...
			if($block['picwidth'] && $block['picheight'] && $curitem['picflag']) { //picflag=0为url地址
				...
				} else if(C::file()->file_exists($_G['setting']['attachdir'].$thumbpath)) {
					$curitem['makethumb'] = 1;
					$curitem['picflag'] = 1;
				}
...
function blockclass_cache() {
	...
	while(($filename=C::file()->readdir($dh))) {
		if(C::file()->is_dir($dir.$filename) && substr($filename,0,1) != '.') {
	...
	foreach($dirs as $name=>$dir) {
		...
		if(C::file()->file_exists($dir.'blockclass.php')) {
		...
		if($blockclass['subs']) {
			...
			$blockstyle = array();
			if(C::file()->file_exists($dir.'blockstyle.php')) {
	...
	if(!empty($bid)) {
		...
		foreach(C::t('common_block_pic')->fetch_all_by_bid_itemid($bid, $itemid) as $value) {
			...
			if($value['picflag']) {
				...
				@C::file()->unlink($_G['setting']['attachdir'].'/'.$value['pic']);
			}
		}
		if(!empty($picids)) {
