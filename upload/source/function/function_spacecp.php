	...
	if($image->Thumb($picsource, 'album/'.$picdir.$albumid.'.jpg', 120, 120, 2)) {
		...
		if(getglobal('setting/ftp/on')) {
			if(ftpcmd('upload', 'album/'.$picdir.$albumid.'.jpg')) {
				$setarr['picflag'] = 2;
				@C::file()->unlink($_G['setting']['attachdir'].'album/'.$picdir.$albumid.'.jpg');
			}
		}
	} else {
		if($pic['status'] == 0) {
...
function pic_save($FILE, $albumid, $title, $iswatermark = true, $catid = 0) {
	...
	if(!$upload->attach['imageinfo'] || !in_array($upload->attach['imageinfo']['2'], array(1,2,3,6))) {
		@C::file()->unlink($upload->attach['target']);
		return lang('spacecp', 'only_allows_upload_file_types');
	}
	...
	if(getglobal('setting/ftp/on')) {
		...
		if($ftpresult) {
			@C::file()->unlink($_G['setting']['attachdir'].'album/'.$upload->attach['attachment']);
			if($thumb) {
				$thumbpath = getimgthumbname($upload->attach['attachment']);
				ftpcmd('upload', 'album/'.$thumbpath);
				@C::file()->unlink($_G['setting']['attachdir'].'album/'.$thumbpath);
			}
			$pic_remote = 1;
			$album_picflag = 2;
		} else {
			if(getglobal('setting/ftp/mirror')) {
				@C::file()->unlink($upload->attach['target']);
				@C::file()->unlink(getimgthumbname($upload->attach['target']));
...
function stream_save($strdata, $albumid = 0, $fileext = 'jpg', $name='', $title='', $delsize=0, $from = false) {
	...
	if($handle = C::file()->fopen($newfilename, 'wb')) {
		if(C::file()->fwrite($handle, $strdata) !== FALSE) {
			C::file()->fclose($handle);

			$size = C::file()->filesize($newfilename);
			...
			if(getglobal('setting/ftp/on')) {
				...
				if($ftpresult) {
					@C::file()->unlink($_G['setting']['attachdir'].'album/'.$filepath);
					if($thumb) {
					...
					if(getglobal('setting/ftp/mirror')) {
						@C::file()->unlink($newfilename);
						@C::file()->unlink(getimgthumbname($newfilename));
						return -3;
					}
				}
			}
