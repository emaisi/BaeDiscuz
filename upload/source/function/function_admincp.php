...
function istpldir($dir) {
	return C::file()->is_dir(DISCUZ_ROOT.'./'.$dir) && !in_array(substr($dir, -1, 1), array('/', '\\')) &&
...
function siteinformation() {
	...
	$updatetime = @C::file()->filemtime(DISCUZ_ROOT.'./data/updatetime.lock');
	if(empty($updatetime) || (TIMESTAMP - $updatetime > 3600 * 4)) {
		@C::file()->touch(DISCUZ_ROOT.'./data/updatetime.lock');
...
function getimportdata($name = '', $addslashes = 0, $ignoreerror = 0) {
	...
	if($_GET['importtype'] == 'file') {
		$data = @implode('', C::file()->file($_FILES['importfile']['tmp_name']));
		@C::file()->unlink($_FILES['importfile']['tmp_name']);
	} else {
...
function siteftp_check($siteftp, $dir) {
	...
	if(!C::file()->file_exists(DISCUZ_ROOT.'./'.$dir.'/discuz_version.php')) {
