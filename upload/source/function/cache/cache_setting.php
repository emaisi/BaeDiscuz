...
function build_cache_setting() {
		...
		foreach($data['verify'] as $key => $value) {
			if($value['available']) {
				if(!empty($value['unverifyicon'])) {
					...
					if(!$icourl['host'] && !C::file()->file_exists($value['unverifyicon'])) {
						$data['verify'][$key]['unverifyicon'] = $data['attachurl'].'common/'.$value['unverifyicon'];
					}
				}
				if(!empty($value['icon'])) {
					$icourl = parse_url($value['icon']);
					if(!$icourl['host'] && !C::file()->file_exists($value['icon'])) {
	...
	foreach($data['watermarktype'] as $k => $v) {
		if($data['watermarktype'][$k] == 'text' && $data['watermarktext']['text'][$k]) {
			...
			if(C::file()->file_exists('static/image/seccode/font/en/'.$data['watermarktext']['fontpath'][$k])) {
				$data['watermarktext']['fontpath'][$k] = 'static/image/seccode/font/en/'.$data['watermarktext']['fontpath'][$k];
			} elseif(C::file()->file_exists('static/image/seccode/font/ch/'.$data['watermarktext']['fontpath'][$k])) {
	...
	if($data['cacheindexlife']) {
		...
		@C::file()->unlink($cachedir.'/'.$tidmd5[0].'/'.$tidmd5[1].'/'.$tidmd5[2].'/0.htm');
	}
...
function writetojscache() {
	...
	while(($entry = C::file()->readdir($dh)) !== false) {
		if(fileext($entry) == 'js') {
			$jsfile = $dir.$entry;
			$fp = C::file()->fopen($jsfile, 'r');
			$jsdata = @C::file()->fread($fp, C::file()->filesize($jsfile));
			C::file()->fclose($fp);
			$jsdata = preg_replace($remove[0], $remove[1], $jsdata);
			if(@$fp = C::file()->fopen(DISCUZ_ROOT.'./data/cache/'.$entry, 'w')) {
				C::file()->fwrite($fp, $jsdata);
				C::file()->fclose($fp);
			} else {
				exit('writetojscache()...Can not write to cache files, please check file:.'.$jsfile);
			}
		}
	}
}
