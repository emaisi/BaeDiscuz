...
function build_cache_smilies_js() {
	...
	if(@$fp = C::file()->fopen($cachedir.'common_smilies_var.js', 'w')) {
		C::file()->fwrite($fp, 'var smthumb = \''.$_G['setting']['smthumb'].'\';'.$return_type.$return_array.'var smilies_fast=['.$return_fast.'];');
		C::file()->fclose($fp);
	} else {
		exit('build_cache_smilies_js()...Can not write to cache files, please check file:'.$cachedir.'common_smilies_var.js');
	}
