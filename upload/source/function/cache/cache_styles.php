	...
	foreach(C::t('common_style')->fetch_all_data(true) as $data) {
		...
		if($data['extstyle']) {
			...
			foreach($extstyle as $dir) {
				if(C::file()->file_exists($extstylefile = DISCUZ_ROOT.$data['tpldir'].'/style/'.$dir.'/style.css')) {
					if($data['defaultextstyle'] == $dir) {
						$data['defaultextstyle'] = $data['tpldir'].'/style/'.$dir;
					}
					$content = C::file()->file_get_contents($extstylefile);
...
function writetocsscache($data) {
	...
	while(($entry = C::file()->readdir($dh)) !== false) {
		if(fileext($entry) == 'css') {
			...
			/*!C::file()->file_exists($cssfile) && */$cssfile = $dir.$entry;
			$cssdata = @implode('', C::file()->file($cssfile));
			if(C::file()->file_exists($cssfile = DISCUZ_ROOT.'./'.$data['tpldir'].'/common/extend_'.$entry)) {
				$cssdata .= @implode('', C::file()->file($cssfile));
			}
			if(is_array($_G['setting']['plugins']['available']) && $_G['setting']['plugins']['available']) {
				foreach($_G['setting']['plugins']['available'] as $plugin) {
					if(C::file()->file_exists($cssfile = DISCUZ_ROOT.'./source/plugin/'.$plugin.'/template/extend_'.$entry)) {
						$cssdata .= @implode('', C::file()->file($cssfile));
					}
				}
			}
			...
			if(@$fp = C::file()->fopen(DISCUZ_ROOT.'./data/cache/style_'.$data['styleid'].'_'.$entry, 'w')) {
				C::file()->fwrite($fp, $cssdata);
				C::file()->fclose($fp);
			} else {
				exit('writetocsscache()...Can not write to cache files, please check file:'.DISCUZ_ROOT.'./data/cache/style_'.$data['styleid'].'_'.$entry);
			}
		}
	}
}
