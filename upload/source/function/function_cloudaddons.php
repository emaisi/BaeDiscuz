...
function cloudaddons_check() {
	...
	foreach(array('download', 'addonmd5') as $path) {
		...
		dmkdir($tmpdir, 0777);
		if(!C::file()->is_dir($tmpdir) || !C::file()->file_exists($tmpfile)) {
			cpmsg('cloudaddons_check_write_error', '', 'error');
		}
		@C::file()->unlink($tmpfile);
		@rmdir($tmpdir);
		if(C::file()->is_dir($tmpdir) || C::file()->file_exists($tmpfile)) {
			cpmsg('cloudaddons_check_write_error', '', 'error');
		}
...
function cloudaddons_getmd5($md5file) {
	$array = array();
	if(preg_match('/^[a-z0-9_\.]+$/i', $md5file) && C::file()->file_exists(DISCUZ_ROOT.'./data/addonmd5/'.$md5file.'.xml')) {
...
function cloudaddons_uninstall($md5file, $dir) {
	...
	@C::file()->unlink(DISCUZ_ROOT.'./data/addonmd5/'.$md5file.'.xml');
...
function cloudaddons_savemd5($md5file, $end, $md5) {
	...
	if(!isset($_G['siteftp'])) {
		...
		$fp = C::file()->fopen(DISCUZ_ROOT.'./data/addonmd5/'.$md5file.'.xml', 'w');
		C::file()->fwrite($fp, array2xml($array));
		C::file()->fclose($fp);
	} else {
		$localfile = DISCUZ_ROOT.'./data/'.random(5);
		$fp = C::file()->fopen($localfile, 'w');
		C::file()->fwrite($fp, array2xml($array));
		C::file()->fclose($fp);
		dmkdir(DISCUZ_ROOT.'./data/addonmd5/', 0777, false);
		siteftp_upload($localfile, 'data/addonmd5/'.$md5file.'.xml');
		@C::file()->unlink($localfile);
	}
}

function cloudaddons_comparetree($new, $old, $basedir, $md5file, $first = 0) {
	global $_G;
	if($first && C::file()->file_exists(DISCUZ_ROOT.'./data/addonmd5/'.$md5file.'.xml')) {
		require_once libfile('class/xml');
		$xml = implode('', @file(DISCUZ_ROOT.'./data/addonmd5/'.$md5file.'.xml'));
		$array = xml2array($xml);
	...
	$dh = C::file()->opendir($new);
	while(($file = C::file()->readdir($dh)) !== false) {
		if($file != '.' && $file != '..') {
			$newfile = $new.'/'.$file;
			$oldfile = $old.'/'.$file;
			if(C::file()->is_file($newfile)) {
				$oldfile = preg_replace('/\._addons_$/', '', $oldfile);
				$md5key = str_replace($basedir, '', preg_replace('/\._addons_$/', '', $newfile));
				$newmd5 = md5_file($newfile);
				$oldmd5 = C::file()->file_exists($oldfile) ? md5_file($oldfile) : '';
...
function cloudaddons_copytree($from, $to) {
	global $_G;
	$dh = C::file()->opendir($from);
	while(($file = C::file()->readdir($dh)) !== false) {
		if($file != '.' && $file != '..') {
			$readfile = $from.'/'.$file;
			$writefile = $to.'/'.$file;
			if(C::file()->is_file($readfile)) {
				if(!in_array($readfile, $_G['treeop']['copy'])) {
					continue;
				}
				if(!isset($_G['siteftp'])) {
					$content = -1;
					if($fp = @C::file()->fopen($readfile, 'r')) {
						$startTime = microtime();
						do {
							$canRead = C::file()->flock($fp, LOCK_SH);
							if(!$canRead) {
						...
						$content = C::file()->fread($fp, C::file()->filesize($readfile));
						C::file()->flock($fp, LOCK_UN);
						C::file()->fclose($fp);
					}
					if($content < 0) {
						cpmsg('cloudaddons_file_read_error', '', 'error');
					...
					if($fp = C::file()->fopen($writefile, 'w')) {
						$startTime = microtime();
						do {
							$canWrite = C::file()->flock($fp, LOCK_EX);
							if(!$canWrite) {
								usleep(round(rand(0, 100) * 1000));
							}
...
function cloudaddons_dirwriteable($basedir, $dir, $sourcedir) {
	...
	foreach($checkdirs as $k => $dir) {
		...
		if(!C::file()->is_dir($checkdir)) {
			@mkdir($checkdir, 0777);
		}
		if(C::file()->is_dir($checkdir)) {
			$fp = C::file()->fopen($checkdir.'/test.txt', 'w');
			if($fp) {
				C::file()->fclose($fp);
				C::file()->unlink($checkdir.'/test.txt');
				$writeable = true;
			} else {
				$writeable = false;
			}
...
function cloudaddons_getsubdirs($dir, $root, &$return) {
	...
	$dh = C::file()->opendir($dir);
	while(($file = C::file()->readdir($dh)) !== false) {
		if($file != '.' && $file != '..') {
			$readfile = $dir.'/'.$file;
			if(C::file()->is_dir($readfile)) {
