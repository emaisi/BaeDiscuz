...
<?php
	...
	if(!submitcheck('editsubmit')) {
		...
		if($_GET['deleteicon']) {
			@C::file()->unlink($_G['setting']['attachurl'].'group/'.$group['icon']);
			$icondata['icon'] = '';
		}
		if($_GET['deletebanner']) {
			@C::file()->unlink($_G['setting']['attachurl'].'group/'.$group['banner']);
			$icondata['banner'] = '';
		}
...
EOT;
		...
		if(!submitcheck('editgrouplevel')) {
			...
			if($_GET['deleteicon']) {
				@C::file()->unlink($_G['setting']['attachurl'].'common/'.$grouplevel['icon']);
				$dataarr['icon'] = '';
			} else {
				if($_FILES['iconnew']) {
...
function delete_groupimg($fidarray) {
	...
	if(!empty($fidarray)) {
		...
		foreach($query as $group) {
			@C::file()->unlink($imgdir.$group['icon']);
			@C::file()->unlink($imgdir.$group['banner']);
		}
	}
}
