...
if($operation == 'filecheck') {
	...
	if($step == 1) {
		...
		foreach($showlist as $file => $md5) {
			...
			if(@array_key_exists($file, $modifylist)) {
				...
				$filemtime = @C::file()->filemtime($file);
			...
			if(C::file()->file_exists($file)) {
				$filemtime = @C::file()->filemtime($file);
				$fileststus && $dirlist[$fileststus][$dir][basename($file)] = array(number_format(C::file()->filesize($file)).' Bytes', dgmdate($filemtime));
			} else {
	...
	if($fp = @C::file()->fopen($attach_dir.'/'.$testfile, 'w')) {
		C::file()->fwrite($fp, $testcontent);
		C::file()->fclose($fp);
	}
	...
	if(!$alertmsg) {
		...
		if(!$alertmsg) {
			...
			@C::file()->unlink($attach_dir.'/'.$testfile);
	...
	if(!empty($_GET['previewthumb'])) {
		...
		@C::file()->unlink(DISCUZ_ROOT.'./data/attachment/temp/watermark_temp1.jpg');
		@C::file()->unlink(DISCUZ_ROOT.'./data/attachment/temp/watermark_temp2.jpg');
		$image = new image;
		$r = 0;
		...
		if($r > 0) {
			...
			$sizesource = C::file()->filesize(DISCUZ_ROOT.'./static/image/admincp/watermarkpreview.jpg');
			echo '<img src="'.C::file()->url().'/data/attachment/temp/watermark_temp1.jpg?'.random(5).'"><br /><br />'.
			...
			echo '<img src="'.C::file()->url().'/data/attachment/temp/watermark_temp2.jpg?'.random(5).'"><br /><br />'.
				$lang['imagepreview_imagesize_source'].' '.number_format($sizesource).' Bytes &nbsp;&nbsp;'.
		...
		require_once libfile('class/image');
		@C::file()->unlink(DISCUZ_ROOT.'./data/attachment/temp/watermark_temp3.jpg');
		$image = new image;
		...
		if($r > 0) {
			...
			$sizesource = C::file()->filesize('static/image/admincp/watermarkpreview.jpg');
			$sizetarget = $image->imginfo['size'];
			echo '<img src="'.C::file()->url().'/data/attachment/temp/watermark_temp3.jpg?'.random(5).'"><br /><br />'.
				$lang['imagepreview_imagesize_source'].' '.number_format($sizesource).' Bytes &nbsp;&nbsp;'.
	...
	if($do == 'output') {
		$robots = implode('', C::file()->file(DISCUZ_ROOT.'./source/admincp/robots.txt'));
...
function checkfiles($currentdir, $ext = '', $sub = 1, $skip = '') {
	global $md5data;
	$dir = @C::file()->opendir(DISCUZ_ROOT.$currentdir);
	$exts = '/('.$ext.')$/i';
	$skips = explode(',', $skip);

	while($entry = @C::file()->readdir($dir)) {
		$file = $currentdir.$entry;
		if($entry != '.' && $entry != '..' && (($ext && preg_match($exts, $entry) || !$ext) || $sub && C::file()->is_dir($file)) && !in_array($entry, $skips)) {
			if($sub && C::file()->is_dir($file)) {
				checkfiles($file.'/', $ext, $sub, $skip);
			} else {
				if(C::file()->is_dir($file)) {
...
function checkcachefiles($currentdir) {
	global $_G;
	$dir = C::file()->opendir($currentdir);
	$exts = '/\.php$/i';
	$showlist = $modifylist = $addlist = array();
	while($entry = C::file()->readdir($dir)) {
		$file = $currentdir.$entry;
		if($entry != '.' && $entry != '..' && preg_match($exts, $entry)) {
			$fp = C::file()->fopen($file, "rb");
			$cachedata = C::file()->fread($fp, C::file()->filesize($file));
			C::file()->fclose($fp);
...
function checkhook($currentdir, $ext = '', $sub = 1, $skip = '') {
	...
	while($entry = C::file()->readdir($dir)) {
		$file = $currentdir.$entry;
		if($entry != '.' && $entry != '..' && (preg_match($exts, $entry) || $sub && C::file()->is_dir($file)) && !in_array($entry, $skips)) {
			if($sub && C::file()->is_dir($file)) {
				checkhook($file.'/', $ext, $sub, $skip);
			} else {
				$data = C::file()->file_get_contents($file);
				$hooks = array();
