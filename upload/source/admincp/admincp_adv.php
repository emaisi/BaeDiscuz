...
<link rel="stylesheet" type="text/css" href="<?php echo C::file()->url().'/data/cache/style_'.$_G['setting']['styleid'].'_common.css';?>" />
</head>
<body>
...
<input type="button" class="btn" name="searchsubmit" value="'.cplang('search').'" onclick="if(this.form.title.value==\''.cplang('adv_inputtitle').'\'){this.form.title.value=\'\'}location.href=\''.ADMINSCRIPT.'?action=adv&operation=ad'.($type ? '&type='.rawurlencode($type) : '').'&title=\'+this.form.title.value+\'&starttime=\'+this.form.starttime.value+\'&endtime=\'+this.form.endtime.value+\'&target=\'+this.form.target.value+\'&orderby=\'+this.form.orderby.value;"> &nbsp;
	...
	if(!submitcheck('advsubmit')) {
		...
		echo '<script type="text/javascript" src="static/js/calendar.js"></script>'.
			...
			(C::file()->file_exists(DISCUZ_ROOT.'./static/image/admincp/'.$type.'.gif') ? '<img src="static/image/admincp/'.$type.'.gif" />' : '').
	...
	if($advs) {
		...
		foreach($advs as $adv) {
			...
			if($adv['class'] == 'custom') {
				...
				echo C::file()->file_exists(DISCUZ_ROOT.'./static/image/admincp/'.$adv['class'].'.gif') ? '<img src="static/image/admincp/'.$adv['class'].'.gif" /><br />' : '';
	...
	if($customadv) {
		$img = C::file()->file_exists(DISCUZ_ROOT.'./static/image/admincp/'.$customadv['class'].'.gif') ? '<img src="static/image/admincp/'.$customadv['class'].'.gif" /><br />' : '';
		$i = $row;
		foreach(C::t('common_advertisement_custom')->fetch_all_data() as $custom) {
			if($i == $row) {
				echo '<tr>';
...
function getadvs() {
	...
	while($entry = $advdir->read()) {
		if(!in_array($entry, array('.', '..')) && preg_match("/^adv\_[\w\.]+$/", $entry) && substr($entry, -4) == '.php' && strlen($entry) < 30 && C::file()->is_file($dir.'/'.$entry)) {
			...
			if(class_exists($advclass)) {
				...
				$advs[$entry] = array(
					...
					'filemtime' => @C::file()->filemtime($dir.'/'.$entry)
				);
			}
		}
	}
