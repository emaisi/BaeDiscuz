...
EOF;
		...
		while($entry = $codedir->read()) {
			if(!in_array($entry, array('.', '..')) && preg_match("/^seccode\_[\w\.]+$/", $entry) && substr($entry, -4) == '.php' && strlen($entry) < 30 && C::file()->is_file($dir.'/'.$entry)) {
...
EOT;
		...
		while($entry = $qaadir->read()) {
			if(!in_array($entry, array('.', '..')) && preg_match("/^secqaa\_[\w\.]+$/", $entry) && substr($entry, -4) == '.php' && strlen($entry) < 30 && C::file()->is_file($dir.'/'.$entry)) {
				@include_once $dir.'/'.$entry;
				...
				if(class_exists($qaaclass)) {
					...
					showtablerow('', array('', 'class="td26"'), array(
						'',
						'<label>'.($qaa->copyright ? '<div class="right">'.lang('secqaa/'.$script, $qaa->copyright).'</div>' : '').'<label><input class="checkbox" class="checkbox" type="checkbox" name="secqaaext[]" value="'.$script.'"'.(in_array($script, $qaaext) ? ' checked="checked"' : '').'> '.lang('secqaa/'.$script, $qaa->name).(@C::file()->filemtime($dir.'/'.$entry) > TIMESTAMP - 86400 ? ' <font color="red">New!</font>' : '').($qaa->description ? '<div class="lightfont" style="margin-left:30px">'.lang('secqaa/'.$script, $qaa->description).'</div>' : '').'</label>'
					));
				}
			}
		}
		...
		$fontlist = array();
		$dir = C::file()->opendir(DISCUZ_ROOT.'./static/image/seccode/font/en');
		...
		$dir = C::file()->opendir(DISCUZ_ROOT.'./static/image/seccode/font/ch');
		while($entry = C::file()->readdir($dir)) {
			if(in_array(strtolower(fileext($entry)), array('ttf', 'ttc'))) {
				$fontlist['portal'] .= '<option value="'.$entry.'"'.($entry == $setting['watermarktext']['fontpath']['portal'] ? ' selected>' : '>').$entry.'</option>';
				$fontlist['forum'] .= '<option value="'.$entry.'"'.($entry == $setting['watermarktext']['fontpath']['forum'] ? ' selected>' : '>').$entry.'</option>';
		...
		$disable = false/*!is_writeable(DISCUZ_ROOT . './config/config_ucenter.php')*/;
	...
	if($operation == 'uc' /*&& is_writeable('./config/config_ucenter.php') */&& $isfounder) {
		require_once C::file()->save_file('./config/config_ucenter.php');
		...
		$fp = C::file()->fopen('./config/config_ucenter.php', 'r');
		$configfile = C::file()->fread($fp, C::file()->filesize('./config/config_ucenter.php'));
		...
		$fp = C::file()->fopen('./config/config_ucenter.php', 'w');
		if(!($fp = @C::file()->fopen('./config/config_ucenter.php', 'w'))) {
			cpmsg('uc_config_write_error', '', 'error');
		}
		@C::file()->fwrite($fp, trim($configfile));
		@C::file()->fclose($fp);
	}
	...
	if(isset($settingnew['seccodedata'])) {
		...
		if($seccoderoot) {
			$dirs = C::file()->opendir($seccoderoot);
			$seccodettf = array();
			while($entry = C::file()->readdir($dirs)) {
	...
	if(isset($settingnew['cachethreaddir']) && isset($settingnew['threadcaches'])) {
		if($settingnew['cachethreaddir'] && !C::file()->is_writable(DISCUZ_ROOT.'./'.$settingnew['cachethreaddir'])) {
...
function watermarkinit($type) {
	...
	if($settingnew['watermarktype'][$type] == 'text' && $settingnew['watermarktext']['fontpath'][$type]) {
		...
		$settingnew['watermarktext']['fontpath'][$type] = C::file()->file_exists('static/image/seccode/font/'.$fontpathnew) ? $fontpathnew : '';
