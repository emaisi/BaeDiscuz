		...
		if($_GET['type'] == 'group') {
			...
			if(!$multiset) {
				if(($_GET['deletebanner'] || $_FILES['bannernew']) && $forum['banner']) {
					$valueparse = parse_url($forum['banner']);
					if(!isset($valueparse['host'])) {
						@C::file()->unlink($_G['setting']['attachurl'].'common/'.$forum['banner']);
					}
				...
				if($_GET['deleteicon'] || $_FILES['iconnew']) {
					$valueparse = parse_url($forum['icon']);
					if(!isset($valueparse['host'])) {
						@C::file()->unlink($_G['setting']['attachurl'].'common/'.$forum['icon']);
					}
					$forumfielddata['icon'] = '';
				...
				if($iconnew) {
					...
					if(!$extranew['iconwidth']) {
						...
						if($info = @getimagesize(C::file()->save_file($iconnew))) {
