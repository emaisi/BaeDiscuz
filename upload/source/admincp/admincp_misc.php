...
EOT;
	...
	if(!submitcheck('stampsubmit')) {
		...
		while($entry = $stampsdir->read()) {
			if(in_array(strtolower(fileext($entry)), $imgextarray) && !in_array($entry, $imgfilter) && C::file()->is_file(DISCUZ_ROOT.'./static/image/stamp/'.$entry)) {
...
<?php
	...
	if(empty($_GET['edit']) && empty($_GET['run'])) {
		...
		if(!submitcheck('cronssubmit')) {
			...
			while($cron = DB::fetch($query)) {
				if(!C::file()->file_exists(DISCUZ_ROOT.'./source/include/cron/'.$cron['filename'])) {
		...
		if(!empty($_GET['edit'])) {
			...
			if(!submitcheck('editsubmit')) {
				...
				} elseif(!C::file()->file_exists(DISCUZ_ROOT.($cronfile = "./source/include/cron/{$_GET['filenamenew']}"))) {
			...
			if(!C::file()->file_exists(DISCUZ_ROOT.($cronfile = "./source/include/cron/$cron[filename]"))) {
