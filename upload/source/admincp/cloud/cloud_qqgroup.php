...
function getTopicThread() {
	...
	if(C::file()->file_exists($_G['setting']['attachdir'].'./'.$thumbTarget)) {
...
function sentMiniportalThreadsRemote($topic, $normal, $gIds = array()) {
	...
	if($topic['extra']['image'] && $topic['extra']['image'] = @C::file()->file_get_contents($_G['setting']['attachdir'].'./'.$topic['extra']['image'])) {
...
function QQGroupUpload($tid) {
	...
	$thumbTarget = $imageDir.'/'.$imageName;
	@C::file()->unlink($_G['setting']['attachdir'].'./'.$thumbTarget);
