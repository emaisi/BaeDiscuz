...
SCRIPT;
	...
	if(!submitcheck('detailsubmit')) {
		...
		if(!empty($foldername)) {
			...
			} elseif(C::file()->is_dir(DISCUZ_ROOT.'./'.$parentdir.$foldername)) {
...
?>";
	$r = C::file()->file_put_contents($dir.'/index.php', $code);
	return $r;
}
...
function delportalcategoryfolder($catid) {
	...
	if(!empty($dir)) {
		C::file()->unlink(DISCUZ_ROOT.$dir.'index.html');
		C::file()->unlink(DISCUZ_ROOT.$dir.'index.php');
...
function remakediytemplate($primaltplname, $targettplname, $diytplname, $olddirectory){
	...
	if(empty($diycontent)) {
		...
		C::file()->file_put_contents(DISCUZ_ROOT.'./data/diy/'.$tpldirectory.'/'.$targettplname.'.htm', $content);
	} else {
