...
if(!$operation) {
	...
	if($step == 0) {
		...
		do {
			...
			if(!$array['Status']) {
				...
				foreach($array['files'] as $file => $data) {
					...
					$fp = C::file()->fopen($filename, !$data['Part'] ? 'w' : 'a');
					if(!$fp) {
						cloudaddons_faillog($_GET['rid'], 101);
						cpmsg('cloudaddons_download_write_error', '', 'error');
					}
					C::file()->fwrite($fp, gzuncompress(base64_decode($data['Data'])));
					C::file()->fclose($fp);
					if($data['MD5']) {
						$md5total .= $data['MD5'];
		...
		if($_GET['type'] == 'plugin') {
			...
			if(C::file()->file_exists(DISCUZ_ROOT.'./data/addonpack/'.$_GET['key'].'.php')) {
