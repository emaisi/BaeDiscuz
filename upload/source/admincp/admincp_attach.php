		...
		for($attachi = 0;$attachi < 10;$attachi++) {
			...
			foreach($attachmentarray as $attachment) {
				if(!$attachment['remote']) {
					$matched = C::file()->file_exists($_G['setting']['attachdir'].'/forum/'.$attachment['attachment']) ? '' : cplang('attach_lost');
					$attachment['url'] = $_G['setting']['attachurl'].'forum/';
				} else {
					@set_time_limit(0);
					if(@C::file()->fclose(@C::file()->fopen($_G['setting']['ftp']['attachurl'].'forum/'.$attachment['attachment'], 'r'))) {
						$matched = '';
					} else {
