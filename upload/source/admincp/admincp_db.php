...
if(!C::file()->is_dir('./data/'.$backupdir)) {
...
if($operation == 'export') {
	...
	if(!submitcheck('exportsubmit', 1)) {
		...
		include C::file()->save_file(DISCUZ_ROOT.'./config/config_ucenter.php');
		...
		if($_GET['method'] == 'multivol') {
			...
			if(trim($sqldump)) {
				...
				@$fp = C::file()->fopen($dumpfilename, 'wb');
				@C::file()->flock($fp, 2);
				if(@!C::file()->fwrite($fp, $sqldump)) {
					@C::file()->fclose($fp);
					cpmsg('database_export_file_invalid', '', 'error');
				} else {
					C::file()->fclose($fp);
					if($_GET['usezip'] == 2) {
						$fp = C::file()->fopen($dumpfilename, "r");
						$content = @C::file()->fread($fp, C::file()->filesize($dumpfilename));
						C::file()->fclose($fp);
						$zip = new zipfile();
						$zip->addFile($content, basename($dumpfilename));
						$fp = C::file()->fopen(sprintf($backupfilename."-%s".'.zip', $volume), 'w');
						if(@C::file()->fwrite($fp, $zip->file()) !== FALSE) {
							@C::file()->unlink($dumpfilename);
						}
						C::file()->fclose($fp);
					}
				...
				if($_GET['usezip'] == 1) {
					...
					for($i = 1; $i <= $volume; $i++) {
						...
						$fp = C::file()->fopen($filename, "r");
						$content = @C::file()->fread($fp, C::file()->filesize($filename));
						C::file()->fclose($fp);
						$zip->addFile($content, basename($filename));
						$unlinks[] = $filename;
						$filelist .= "<li><a href=\"$filename\">$filename</a></li>\n";
					}
					$fp = C::file()->fopen($zipfilename, 'w');
					if(@C::file()->fwrite($fp, $zip->file()) !== FALSE) {
					...
					unset($sqldump, $zip, $content);
					C::file()->fclose($fp);
					@C::file()->touch('./data/'.$backupdir.'/index.htm');
			...
			require C::file()->save_file(DISCUZ_ROOT . './config/config_global.php');
			...
			@C::file()->unlink($dumpfile);

			$mysqlbin = $mysql_base == '/' ? '' : addslashes($mysql_base).'bin/';
			@shell_exec($mysqlbin.'mysqldump --force --quick '.($db->version() > '4.1' ? '--skip-opt --create-options' : '-all').' --add-drop-table'.($_GET['extendins'] == 1 ? ' --extended-insert' : '').''.($db->version() > '4.1' && $_GET['sqlcompat'] == 'MYSQL40' ? ' --compatible=mysql40' : '').' --host="'.$dbhost.($dbport ? (is_numeric($dbport) ? ' --port='.$dbport : ' --socket="'.$dbport.'"') : '').'" --user="'.$dbuser.'" --password="'.$dbpw.'" "'.$dbname.'" '.$tablesstr.' > '.$dumpfile);

			if(@C::file()->file_exists($dumpfile)) {

				if($_GET['usezip']) {
					require_once libfile('class/zip');
					$zip = new zipfile();
					$zipfilename = $backupfilename.'.zip';
					$fp = C::file()->fopen($dumpfile, "r");
					$content = @C::file()->fread($fp, C::file()->filesize($dumpfile));
					C::file()->fclose($fp);
					$zip->addFile($idstring."# <?php exit();?>\n ".$setnames."\n #".$content, basename($dumpfile));
					$fp = C::file()->fopen($zipfilename, 'w');
					@C::file()->fwrite($fp, $zip->file());
					C::file()->fclose($fp);
					@C::file()->unlink($dumpfile);
					@C::file()->touch('./data/'.$backupdir.'/index.htm');
					...
					if(@is_writeable($dumpfile)) {
						$fp = C::file()->fopen($dumpfile, 'rb+');
						@C::file()->fwrite($fp, $idstring."# <?php exit();?>\n ".$setnames."\n #");
						C::file()->fclose($fp);
					}
					@C::file()->touch('./data/'.$backupdir.'/index.htm');
	...
	if(!submitcheck('deletesubmit')) {
		...
		if(C::file()->is_dir(DISCUZ_ROOT.'./data/'.$backupdir)) {
			$dir = dir(DISCUZ_ROOT.'./data/'.$backupdir);
			while($entry = $dir->read()) {
				$entry = './data/'.$backupdir.'/'.$entry;
				if(C::file()->is_file($entry)) {
					if(preg_match("/\.sql$/i", $entry)) {
						$filesize = C::file()->filesize($entry);
						$fp = C::file()->fopen($entry, 'rb');
						...
						$exportlog[$key][$identify[4]] = array(
							...
							'volume' => $identify[4],
							'filename' => $entry,
							'dateline' => C::file()->filemtime($entry),
							'size' => $filesize
						);
		...
		if(is_array($_GET['delete'])) {
			foreach($_GET['delete'] as $filename) {
				...
				if(C::file()->is_file($file_path)) {
					@C::file()->unlink($file_path);
				} else {
					$i = 1;
					while(1) {
	...
	if($step == 3) {
		...
		if(!C::file()->file_exists('source/admincp/discuzdb.md5')) {
		...
		$fp = C::file()->fopen(DISCUZ_ROOT.'./source/admincp/discuzdb.md5', "rb");
		$discuzdb = C::file()->fread($fp, C::file()->filesize(DISCUZ_ROOT.'./source/admincp/discuzdb.md5'));
		C::file()->fclose($fp);
		...
		if(submitcheck('repairsubmit') && (!empty($repair) || !empty($setting) || !empty($repairtable) || !empty($missingtable))) {
			...
			foreach($missingtable as $value) {
				if(!isset($installdata)) {
					$fp = C::file()->fopen(DISCUZ_ROOT.'./install/install.sql', "rb");
					$installdata = C::file()->fread($fp, C::file()->filesize(DISCUZ_ROOT.'./install/install.sql'));
					C::file()->fclose($fp);
				}
		...
		$installexists = C::file()->file_exists(DISCUZ_ROOT.'./install/install.sql');
