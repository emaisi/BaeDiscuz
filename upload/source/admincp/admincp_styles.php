...
if($operation == 'admin') {
	...
	if(!submitcheck('stylesubmit')) {
		...
		while($entry = $templatedir->read()) {
			...
			if(!in_array($entry, array('.', '..')) && !in_array($tpldir, $tpldirs) && C::file()->is_dir($tpldir)) {
				$styleexist = 0;
		...
		foreach($sarray as $id => $style) {
			...
			$preview = C::file()->file_exists($style['directory'].'/preview.jpg') ? $style['directory'].'/preview.jpg' : './static/image/admincp/stylepreview.gif';
			$previewlarge = C::file()->file_exists($style['directory'].'/preview_large.jpg') ? $style['directory'].'/preview_large.jpg' : '';
	...
	if(!submitcheck('editsubmit')) {
		...
		if(C::file()->file_exists($extstyledir = DISCUZ_ROOT.$style['directory'].'/style')) {
			$defaultextstyle[] = array('', $lang['default']);
			$tpl = dir($extstyledir);
			while($entry = $tpl->read()) {
				if($entry != '.' && $entry != '..' && C::file()->file_exists($extstylefile = $extstyledir.'/'.$entry.'/style.css')) {
