...
if(!$operation) {
	...
	if(!submitcheck('submit')) {
		...
		if(empty($_GET['system'])) {
			...
			while($entry = $pluginsdir->read()) {
				if(!in_array($entry, array('.', '..')) && C::file()->is_dir($plugindir.'/'.$entry) && !in_array($entry, $plugins)) {
					$entrydir = DISCUZ_ROOT.'./source/plugin/'.$entry;
					$d = dir($entrydir);
					$filemtime = C::file()->filemtime($entrydir);
					$entrytitle = $entry;
					$entryversion = $entrycopyright = $importtxt = '';
					$extra = currentlang();
					$extra = $extra ? '_'.$extra : '';
					if(C::file()->file_exists($entrydir.'/discuz_plugin_'.$entry.$extra.'.xml')) {
						$importtxt = @implode('', C::file()->file($entrydir.'/discuz_plugin_'.$entry.$extra.'.xml'));
					} elseif(C::file()->file_exists($entrydir.'/discuz_plugin_'.$entry.'.xml')) {
						$importtxt = @implode('', C::file()->file($entrydir.'/discuz_plugin_'.$entry.'.xml'));
					}
					if($importtxt) {
	...
	if($modules['extra']['langexists'] && C::file()->file_exists($file = DISCUZ_ROOT.'./data/plugindata/'.$pluginarray['plugin']['identifier'].'.lang.php')) {
		include $file;
	...
	if(C::file()->file_exists($plugindir.'/install.php')) {
		$pluginarray['installfile'] = 'install.php';
	}
	if(C::file()->file_exists($plugindir.'/uninstall.php')) {
		$pluginarray['uninstallfile'] = 'uninstall.php';
	}
	if(C::file()->file_exists($plugindir.'/upgrade.php')) {
		$pluginarray['upgradefile'] = 'upgrade.php';
	}
	if(C::file()->file_exists($plugindir.'/check.php')) {
	...
	if(submitcheck('importsubmit') || isset($_GET['dir'])) {
		...
		if(!isset($_GET['dir'])) {
			...
			$importtxt = @implode('', C::file()->file($importfile));
			$pluginarray = getimportdata('Discuz! Plugin');
		...
		if(!empty($pluginarray['checkfile']) && preg_match('/^[\w\.]+$/', $pluginarray['checkfile'])) {
			...
			if(C::file()->file_exists($filename)) {
	...
	if(!C::file()->file_exists($importfile)) {
	...
	if(!$_GET['confirmed']) {
		...
		if(C::file()->file_exists($file)) {
			$importtxt = @implode('', C::file()->file($file));
			$pluginarray = getimportdata('Discuz! Plugin');
		...
		$upgradestr = '';
		if(C::file()->file_exists($entrydir)) {
			$d = dir($entrydir);
			while($f = $d->read()) {
				if(preg_match('/^discuz\_plugin\_'.$plugin['identifier'].'(\_\w+)?\.xml$/', $f, $a)) {
					...
					$importtxt = @implode('', C::file()->file($entrydir.'/'.$f));
					$pluginarray = getimportdata('Discuz! Plugin');
	...
	if(!submitcheck('editsubmit')) {
		...
		if($type == 'common') {
			...
			if($_GET['langexists'] && !C::file()->file_exists($langfile = DISCUZ_ROOT.'./data/plugindata/'.$identifiernew.'.lang.php')) {
