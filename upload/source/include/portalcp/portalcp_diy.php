...
if (submitcheck('uploadsubmit')) {
		...
		if(getglobal('setting/ftp/on')) {
			if(ftpcmd('upload', 'portal/'.$attach['attachment'])) {
				...
				if(getglobal('setting/ftp/mirror')) {
					@C::file()->unlink($attach['target']);
					@C::file()->unlink(getimgthumbname($attach['target']));
	...
	if($optype == 'canceldiy') {
		@C::file()->unlink(DISCUZ_ROOT.'./data/diy/'.$tpldirectory.'/'.$targettplname.'_diy_preview.htm');
		if($targettplname == $template) @C::file()->unlink(DISCUZ_ROOT.'./data/diy/'.$tpldirectory.'/'.$targettplname.'_'.$clonefile.'_diy_preview.htm');
		showmessage('do_success');
	}

	if ($recover == '1') {
		$file = './data/diy/'.$tpldirectory.'/'.$targettplname.'.htm';
		if (C::file()->is_file($file.'.bak')) {
			copy ($file.'.bak', $file);
		} else {
		...
		if ($r && $optype != 'savecache') {
			if (!$iscategory && !$istopic && empty($savemod) && !empty($clonefile)) {
				...
				if (C::file()->file_exists($delfile)) {
					C::file()->unlink($delfile);
					@C::file()->unlink($delfile.'.bak');
...
if($op == 'blockclass') {
	...
	if ($_GET['type'] == 1) {
		...
		if(($dh = @C::file()->opendir($xmlfilepath))) {
...
function getdiyxmlname($filename, $path) {
	$content = @C::file()->file_get_contents($path.$filename);
	$name = $filename;
	if($content) {
