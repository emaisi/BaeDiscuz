...
@C::file()->touch($_G['setting']['attachdir'].'image/index.htm');
...
function removedir($dirname, $keepdir = FALSE) {
	...
	if(!C::file()->is_dir($dirname)) {
		return FALSE;
	}
	$handle = C::file()->opendir($dirname);
	while(($file = C::file()->readdir($handle)) !== FALSE) {
		if($file != '.' && $file != '..') {
			$dir = $dirname . DIRECTORY_SEPARATOR . $file;
			C::file()->is_dir($dir) ? removedir($dir) : C::file()->unlink($dir);
		}
	}
