...
class QRtools {
	...
	public static function log($outfile, $err) {
		if (QR_LOG_DIR !== false) {
			if ($err != '') {
				if ($outfile !== false) {
					C::file()->file_put_contents(QR_LOG_DIR . basename($outfile) . '-errors.txt', date('Y-m-d H:i:s') . ': ' . $err, FILE_APPEND);
				} else {
					C::file()->file_put_contents(QR_LOG_DIR . 'errors.txt', date('Y-m-d H:i:s') . ': ' . $err, FILE_APPEND);
				}
			}
		}
	}
		...
		if (!isset(self::$frames[$version])) {
			...
			if (QR_CACHEABLE) {
				if (C::file()->file_exists($fileName)) {
					self::$frames[$version] = self::unserial(C::file()->file_get_contents($fileName));
				} else {
					self::$frames[$version] = self::createFrame($version);
					C::file()->file_put_contents($fileName, self::serial(self::$frames[$version]));
				}
			} else {
				self::$frames[$version] = self::createFrame($version);
			}
...
class QRmask {
	...
	public function makeMaskNo($maskNo, $width, $s, &$d, $maskGenOnly = false) {
		...
		if (QR_CACHEABLE) {
			if (C::file()->file_exists($fileName)) {
				$bitMask = self::unserial(C::file()->file_get_contents($fileName));
			} else {
				$bitMask = $this->generateMaskNo($maskNo, $width, $s, $d);
				if (!C::file()->file_exists(QR_CACHE_DIR . 'mask_' . $maskNo))
					mkdir(QR_CACHE_DIR . 'mask_' . $maskNo);
				C::file()->file_put_contents($fileName, self::serial($bitMask));
			}
		} else {
			$bitMask = $this->generateMaskNo($maskNo, $width, $s, $d);
		}
...
class QRencode {
	...
	public function encode($intext, $outfile = false) {
		...
		if ($outfile !== false) {
			C::file()->file_put_contents($outfile, join("\n", QRtools::binarize($code->data)));
		} else {
