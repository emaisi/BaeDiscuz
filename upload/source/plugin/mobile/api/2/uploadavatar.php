...
class mobile_api {
	...
	function common() {
		...
		C::file()->file_exists($tmpavatar) && @C::file()->unlink($tmpavatar);
		if(@copy($_FILES['Filedata']['tmp_name'], $tmpavatar) || @move_uploaded_file($_FILES['Filedata']['tmp_name'], $tmpavatar)) {
			@C::file()->unlink($_FILES['Filedata']['tmp_name']);
			list($width, $height, $type, $attr) = getimagesize($tmpavatar);
			if($width < 10 || $height < 10 || $type == 4) {
				@C::file()->unlink($tmpavatar);
	...
	function output() {
		...
		if(!empty($_G['uid'])) {
			if($this->tmpavatarbig && $this->tmpavatarmiddle && $this->tmpavatarsmall) {
				$avatar1 = self::byte2hex(C::file()->file_get_contents($this->tmpavatarbig));
				$avatar2 = self::byte2hex(C::file()->file_get_contents($this->tmpavatarmiddle));
				$avatar3 = self::byte2hex(C::file()->file_get_contents($this->tmpavatarsmall));

				$extra = '&avatar1='.$avatar1.'&avatar2='.$avatar2.'&avatar3='.$avatar3;
				$result = self::uc_api_post_ex('user', 'rectavatar', array('uid' => $_G['uid']), $extra);

				@C::file()->unlink($this->tmpavatar);
				@C::file()->unlink($this->tmpavatarbig);
				@C::file()->unlink($this->tmpavatarmiddle);
				@C::file()->unlink($this->tmpavatarsmall);
